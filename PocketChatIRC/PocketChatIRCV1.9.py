...
class Window(Frame):
    ...
    def getuser_popup(self):# Builds the UI for the username entry window
        ...
        self.usernameButton.place(x=75, y=140)
    ...
    def get_incoming_channel(self, line):
        if "#" in line[2]: # Message from channel
            ...
            user = finish_format[0].lower()
            self.add_tab(user, type)
            self.find_window(user, user + ':>' + message + ' ' + x + '\n')
    ...
    def add_tab(self, tab_name, type):        
        if tab_name not in self.tabs:
            ...
            for i in self.n.tabs():
                if tab_name == self.n.tab(i, "text") and self.n.tab(i, "state") == 'hidden' and type == 'channel':
                ...
                elif tab_name == self.n.tab(i, "text") and self.n.tab(i, "state") == 'normal' and type == 'channel':
                    self.n.select(i)
                    self.tabs[tab_name]['entrybox'].focus_force()
