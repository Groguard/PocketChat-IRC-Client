...
class Window(Frame):
    ...
    def init_window(self):# Builds the UI for the main window
        ...
        self.tabs[self.SERVER] = {}
        self.tabs[self.SERVER]['textbox'] = self.textReceive
        self.tabs[self.SERVER]['entrybox'] = self.textEntry
        self.tabs[self.SERVER]['onlineusers'] = ''
        
        self.n.add(self.textboxframe, text=self.SERVER)
    ...
    def getuser_popup(self):# Builds the UI for the username entry window
        ...
        self.enterPASSWORD = Entry(self.top, show="*", width=18)
        self.enterPASSWORD.place(x=75, y=76)
        self.enterPASSWORDLb = Label(self.top, text = '*Password')
        self.enterPASSWORDLb.place(x=5, y=76)
        
        self.sslCHECKBOX = Checkbutton(self.top, variable=self.checkSSL, onvalue=1, offvalue=0, text="Use SSL")
        self.sslCHECKBOX.place(x=5, y=96)         
        self.sslCHECKBOX.select()
        
        self.OPTIONALLb = Label(self.top, text = '"*"optional')
        self.OPTIONALLb.place(x=135, y=96)
        ...
        self.usernameButton.place(x=70, y=120)
    ...
    def process_commands(self, message):
        ...
        if '/msg' in format:
            ...
            if len(message) < 3:
                ...
                self.add_pm_tab(tab, self.aliasName + ':>' + message + '\n')
                self.tabs[tab]['entrybox'].delete('1.0', END)
                self.sckt.send(bytes("PRIVMSG %s %s \r\n" % (tab, message), "UTF-8"))     
        ...
        while True:
            ...
            if readable:
                ...
                for line in temp:
                    ...
                    line=str.split(line)
                    time.sleep(0.002)
                    self.master.after_idle(self.iterate_through_incoming, line)
    ...
    def iterate_through_incoming(self, line): # Look through the incoming messages for info from the server
        ...
        except IndexError: # Some times there is no data in some positions of the incoming messages
            pass
    ...
    def get_incoming_channel(self, line):
        if "#" in line[2]: # Message from channel
            ...
            user = "" # Get the incoming sender   
            message = line[3].lstrip(':')  
            x = " ".join(line[4:]) # Grabbing everything after the 3rd index and joining the message.
            format_sender1 = line[0].strip(':')
            finish_format = format_sender1.split('!')
            user = finish_format[0]
            self.add_pm_tab(user, user + ':>' + message + ' ' + x + '\n') # Add a tab and insert the incoming text into it
            
    def add_tab(self, channel):        
        if channel not in self.tabs:
            self.tab_generator(channel)
        else:
            for i in self.n.tabs():
                if channel == self.n.tab(i, "text") and self.n.tab(i, "state") == 'hidden':
                    self.n.tab(i, state='normal')
                    self.n.select(i)
                    self.tabs[channel]['onlineusers'].delete(1, 'end')
                    self.tabs[channel]['textbox'].delete(0,'end-1c')
                    self.tabs[channel]['entrybox'].focus_force()
                    self.sckt.send(bytes("JOIN %s\r\n" % channel, "UTF-8"))
                elif channel == self.n.tab(i, "text") and self.n.tab(i, "state") == 'normal':
                    self.n.select(i) # If the incoming user in the the current tabs change the state of the tab to normal if hidden 
                    self.tabs[channel]['entrybox'].focus_force()        
                    
    def add_pm_tab(self, tab, message): # Adds tabs and routes the messages for incoming pms
        if tab not in self.tabs: # if the user isn't in the the dictionary generate a new tab
            # Create Client Tab
            self.pm_tab_generator(tab)
            self.find_window(tab, message)
        else: # if the user is in the dictionary, find the window and insert the text to the window
            for i in self.n.tabs():
                if tab == self.n.tab(i, "text"):
                    self.n.select(i)
                    self.tabs[tab]['entrybox'].focus_force()        
                    self.find_window(tab, message) 
            
    def tab_generator(self, tab_name): # Tab generator for joining channels
        # Create channel tab
    ...
    def pm_tab_generator(self, user): # Tab generator for PMs from users
        # Create PM tab
        self.user = ttk.Frame(self.n)
        self.user.grid(row=0, column=0, rowspan=2, sticky=N+S+E+W)
        
        self.receive_user = ScrolledText(self.user,  height=24, width=47, wrap = WORD)
        self.receive_user.grid(row=0, column=0, padx=(10,0), pady=(10,5), sticky=N+S+E+W)
        self.receive_user.config(state=DISABLED)
        
        self.pm_Entry = ScrolledText(self.user, height=2, width=47, wrap = WORD)
        self.pm_Entry.grid(row=2, column=0, padx=(10,0), pady=(0,10), sticky=N+S+E+W)
        self.pm_Entry.bind('<Return>', self.check_pm_commands)
        
        self.tabs[user] = {}
        self.tabs[user]['textbox'] = self.receive_user
        self.tabs[user]['entrybox'] = self.pm_Entry
        self.tabs[user]['onlineusers'] = ''
        
        self.pm_Close = Button(self.user, width=7, text='Close tab', command=lambda:self.remove_on_close())
        self.pm_Close.grid(row=0, column=1, padx=(5,5), pady=(5,150), sticky=N+S+E+W)      
        
        Grid.rowconfigure(self.user, 0, weight=1)
        Grid.columnconfigure(self.user, 0, weight=1)
            
        self.n.add(self.user, text = user)
        self.n.select(self.user)
        self.pm_Entry.focus_force()
    ...
    def find_window(self, tab, message): # Get the name of the tab so the message gets to the correct tab
        for i in self.n.tabs():
            if tab == self.n.tab(i, "text"):
                if "No such channel" in self.tabs[tab]['textbox'].get("1.0",'end-1c'):
                    pass
                else:
                    self.n.tab(i, state='normal') # If the incoming user in the the current tabs change the state of the tab to normal if hidden
                    if tab in self.tabs: # Find the window in the dictionary and insert the message there
                        self.post_pm_controls(message, self.tabs[tab]['textbox'])# Post the received text to the window
    ...
    def remove_online_user(self, user, tab): # Remove users as they leave
    ...
    def count_online(self, tab):
        user_count = self.tabs[tab]['onlineusers'].size()
        self.tabs[tab]['onlineusers'].delete(0)
        self.tabs[tab]['onlineusers'].insert(0, 'Online [%d]' % (user_count - 1))       
